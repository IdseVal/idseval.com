...
const ArticleListENG = (props) => {
  ...
  useEffect(() => {
    const getAllQuotes = async () => {
      ...
      const loadedArticlesENG = loadedArticles.filter(
        (article) => article.language === "ENG"
      );

      setList(loadedArticlesENG);
      setIsLoading(false);
    };
    getAllQuotes();
  }, []);
